...
def informedSearch(graphObj, startNodeObj, goalNodeObj, startNode_gcost, startNode_hcost, costFunction):
    ...
    prtuple = Priority(startNode_gcost + startNode_hcost, 0, startNodeObj, startNode_gcost, None)
    ...
    while open_q:
        ...
        for edge in children:
            ...
            if not childFromOpenQueue and not edge.child in closed:
                ...
                prtuple = Priority(costFunction(node_tuple, edge), edge.order, edge.child, fetchgcost(node_tuple, edge),
                                   node_tuple.node)
                heapq.heappush(open_q, prtuple)
                ...
                if childFromOpenQueue.fcost > currentCost:
                    newPriority = Priority(currentCost, edge.order, edge.child, fetchgcost(node_tuple, edge),
                                           node_tuple.node)
                    if newPriority:
                        for i in open_q:
